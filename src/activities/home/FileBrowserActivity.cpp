...
int FileBrowserActivity::compactPageEnd(const int startIndex, const int contentHeight) {
  const int listSize = static_cast<int>(entryCount());
  if (listSize <= 0) return 0;

  const auto& metrics = UITheme::getInstance().getMetrics();
  const int fileRowHeight = MinimalTheme::compactFileBrowserRowHeightFor(renderer);
  const int folderRowHeight = mappedInput.hasTouch() ? fileRowHeight : metrics.listRowHeight;
  if (mappedInput.hasTouch()) {
    const int pageItems = std::max(1, contentHeight / fileRowHeight);
    return std::min(listSize, std::clamp(startIndex, 0, listSize) + pageItems);
  }
  int usedHeight = 0;
  int endIndex = std::clamp(startIndex, 0, listSize);
  while (endIndex < listSize) {
    const char* entry = entryNameAt(static_cast<size_t>(endIndex));
    const int rowHeight =
        (entry != nullptr && entry[0] != '\0' && entry[strlen(entry) - 1] == '/') ? folderRowHeight : fileRowHeight;
    if (endIndex > startIndex && usedHeight + rowHeight > contentHeight) break;
    usedHeight += rowHeight;
    endIndex++;
  }
  return std::max(std::min(startIndex + 1, listSize), endIndex);
}

int FileBrowserActivity::compactPageStartForSelection(const int contentHeight) {
  const int listSize = static_cast<int>(entryCount());
  if (listSize <= 0) return 0;

  const int selected = std::clamp(static_cast<int>(selectorIndex), 0, listSize - 1);
  int pageStart = 0;
  int pageEnd = compactPageEnd(pageStart, contentHeight);
  while (selected >= pageEnd && pageEnd < listSize) {
    pageStart = pageEnd;
    pageEnd = compactPageEnd(pageStart, contentHeight);
  }
  return pageStart;
}

int FileBrowserActivity::pageItemsForList(const int contentHeight, const bool compactFileRows) {
  if (!compactFileRows) {
    return UITheme::getInstance().getTheme().getListPageItems(contentHeight, false);
  }

  const int pageStart = compactPageStartForSelection(contentHeight);
  return compactPageEnd(pageStart, contentHeight) - pageStart;
}

bool FileBrowserActivity::entryIndexFromPoint(const int x, const int y, const int contentTop, const int contentHeight,
                                              const bool compactFileRows, int& index) {
  const int listSize = static_cast<int>(entryCount());
  if (listSize <= 0) return false;
  if (y < contentTop || y >= contentTop + contentHeight) return false;

  if (!compactFileRows) {
    (void)x;
    const auto& theme = UITheme::getInstance().getTheme();
    const int rowStep = theme.getListRowStep(false);
    const int pageItems = theme.getListPageItems(contentHeight, false);
    if (rowStep <= 0 || pageItems <= 0) return false;
    const int pageStart = std::max(0, static_cast<int>(selectorIndex) / pageItems) * pageItems;
    const int row = (y - contentTop) / rowStep;
    const int touched = pageStart + row;
    if (row < 0 || row >= pageItems || touched >= listSize) return false;
    index = touched;
    return true;
  }
  ...
  const int folderRowHeight = mappedInput.hasTouch() ? fileRowHeight : metrics.listRowHeight;
  const int pageStart = compactPageStartForSelection(contentHeight);
  const int pageEnd = compactPageEnd(pageStart, contentHeight);
  if (mappedInput.hasTouch()) {
    const int row = (y - contentTop) / fileRowHeight;
    const int touched = pageStart + row;
    if (touched >= pageEnd) return false;
    index = touched;
    return true;
  }
  int itemY = contentTop;
  for (int i = pageStart; i < pageEnd; ++i) {
    const char* entry = entryNameAt(static_cast<size_t>(i));
    const int rowHeight =
        (entry != nullptr && entry[0] != '\0' && entry[strlen(entry) - 1] == '/') ? folderRowHeight : fileRowHeight;
...
size_t FileBrowserActivity::nextTouchPageIndex(const int listSize, const int contentHeight,
                                               const bool compactFileRows) {
  if (listSize <= 0) return 0;
  if (!compactFileRows) {
    return static_cast<size_t>(ButtonNavigator::nextPageIndex(static_cast<int>(selectorIndex), listSize,
                                                              pageItemsForList(contentHeight, compactFileRows)));
  }

  const int pageStart = compactPageStartForSelection(contentHeight);
  const int pageEnd = compactPageEnd(pageStart, contentHeight);
  return static_cast<size_t>(pageEnd < listSize ? pageEnd : 0);
}

size_t FileBrowserActivity::previousTouchPageIndex(const int listSize, const int contentHeight,
                                                   const bool compactFileRows) {
  if (listSize <= 0) return 0;
  if (!compactFileRows) {
    return static_cast<size_t>(ButtonNavigator::previousPageIndex(static_cast<int>(selectorIndex), listSize,
                                                                  pageItemsForList(contentHeight, compactFileRows)));
  }
  ...
  if (pageStart <= 0) {
    int lastPageStart = 0;
    int nextPageStart = compactPageEnd(lastPageStart, contentHeight);
    while (nextPageStart < listSize) {
      lastPageStart = nextPageStart;
      nextPageStart = compactPageEnd(lastPageStart, contentHeight);
    }
    return static_cast<size_t>(lastPageStart);
  }

  int previousPageStart = 0;
  int nextPageStart = compactPageEnd(previousPageStart, contentHeight);
  while (nextPageStart < pageStart) {
    previousPageStart = nextPageStart;
    nextPageStart = compactPageEnd(previousPageStart, contentHeight);
  }
  return static_cast<size_t>(previousPageStart);
}
