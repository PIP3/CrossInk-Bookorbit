...
void relayout(PreviewLayout& layout, const GfxRenderer& renderer, int fontId, int textWidth) {
  ...
  style.textIndent = static_cast<int16_t>(renderer.getSpaceWidth(fontId, EpdFontFamily::REGULAR) * 3);
  style.textIndentDefined = true;  // model a book-provided indent so Force Paragraph Indents is visible

  ParsedText parsed(SETTINGS.extraParagraphSpacing != 0, SETTINGS.forceParagraphIndents != 0,
                    SETTINGS.hyphenationEnabled != 0, SETTINGS.bionicReadingEnabled != 0,
                    SETTINGS.guideReadingEnabled != 0, SETTINGS.wordSpacing, style);
...
void renderPreview(const GfxRenderer& renderer, PreviewLayout& layout, int previewPadding, int labelGap, int top,
  ...
  // sample text, styleMask<-bionicReading), all of which are key fields, so a matching
  ...
  const PreviewKey key{.fontId = fontId,
                       ...
                       .bionicReading = SETTINGS.bionicReadingEnabled != 0,
                       .guideReading = SETTINGS.guideReadingEnabled != 0,
                       .hyphenation = SETTINGS.hyphenationEnabled != 0};
  if (key != layout.key) {
    if (auto* fcm = renderer.getFontCacheManager()) {
      fcm->prewarmCache(fontId, I18N.get(StrId::STR_FONT_PREVIEW_TEXT), SETTINGS.bionicReadingEnabled ? 0x03 : 0x01);
    }
