...
class TextSettingsActivity final : public Activity {
 public:
  ...
  TextSettingsActivity(GfxRenderer& renderer, MappedInputManager& mappedInput, const SdCardFontRegistry* registry,
                       Tab initialTab = Tab::Family, bool readerActivity = false);

  void onEnter() override;
  void onExit() override;
  void loop() override;
  void render(RenderLock&&) override;
  bool allowPowerAsConfirmInReaderMode() const override { return readerActivity_; }
  bool allowGlobalHomeGesture() const override { return !readerActivity_; }

 private:
  // Row indices per tab. enum class (not plain enum) so a LayoutRow can't be
  // silently confused with a StyleRow of equal value.
  enum class LayoutRow { LineSpacing, WordSpacing, ParaSpacing, ForceIndents, Alignment, ScreenMargin, Count };
  enum class StyleRow { BionicReading, GuideReading, Hyphenation, EmbeddedStyle, AntiAliasing, Count };
