...
void QuickActionsActivity::showOverview() {
  ...
  for (uint8_t i = 0; i < 5; ++i) {
    ...
    const char* label = action < CrossPointSettings::QUICK_ACTIONS ? I18N.get(actionLabels[action]) : "-";
...
void QuickActionsActivity::editSlot(uint8_t slot) {
  std::vector<std::string> labels;
  labels.reserve(CrossPointSettings::QUICK_ACTIONS);
  for (const auto label : actionLabels) labels.emplace_back(I18N.get(label));
  uint8_t current = SETTINGS.quickActionSlots[slot];
  if (current >= CrossPointSettings::QUICK_ACTIONS) current = CrossPointSettings::IGNORE;
